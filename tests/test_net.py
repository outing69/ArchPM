"""Tests for archpm.net against synthetic `ss -tunapiH` output (real format, invented data)."""
...
SS = """\
udp UNCONN 0 0 224.0.0.251:5353 0.0.0.0:* users:(("helperd",pid=1201,fd=182))
udp UNCONN 0 0 0.0.0.0:5353 0.0.0.0:*
udp UNCONN 0 0 *:1716 *:* users:(("pairingd",pid=1302,fd=16))
udp ESTAB 0 0 192.0.2.10%wlan0:68 192.0.2.1:67
tcp LISTEN 0 128 0.0.0.0:27036 0.0.0.0:* users:(("launcher",pid=1403,fd=155))
tcp LISTEN 0 128 127.0.0.1:57343 0.0.0.0:* users:(("launcher",pid=1403,fd=60))
tcp ESTAB 0 0 192.0.2.10:46516 198.51.100.7:443 users:(("uplink",pid=1504,fd=17))
\t cubic wscale:8,10 rto:219 mss:1400 bytes_sent:2063 bytes_acked:2064 bytes_received:1454
tcp ESTAB 0 0 127.0.0.1:57634 127.0.0.1:34175 users:(("helperd",pid=1205,fd=30))
\t cubic wscale:2,10 rto:203 bytes_sent:5964 bytes_acked:5965 bytes_received:20632 segs_out:1291
tcp ESTAB 0 0 [2001:db8:1::5]:51234 [2001:db8:2::200e]:443 users:(("webview",pid=1606,fd=99))
\t cubic bytes_acked:1000 bytes_received:50000
tcp TIME-WAIT 0 0 192.0.2.10:40000 198.51.100.20:443
"""
...
class Parsing(unittest.TestCase):
    def test_sockets_and_owners(self):
        ...
        kde = next(c for c in conns if c.pid == 1302)
    ...
    def test_counters_follow_their_socket(self):
        ...
        key = next(k for k in counters if k[5] == 1504)
        self.assertEqual(counters[key], (1454, 2064))   # (received, acked)

    def test_split_addr(self):
        self.assertEqual(net.split_addr("192.0.2.10%wlan0:46516"), ("192.0.2.10", 46516))
...
class Rates(unittest.TestCase):
    def test_tcp_rates_from_two_scans(self):
        ...
        a = s.sample({1504: "Uplink"})
        self.assertEqual(a.procs[1504].rx_bps, 0.0, "no rate on the first scan")
        s._prev_ts -= 10.0                       # pretend ten seconds passed
        b = s.sample({1504: "Uplink"})
        self.assertAlmostEqual(b.procs[1504].rx_bps, 5000.0, delta=50)
        self.assertAlmostEqual(b.procs[1504].tx_bps, 1000.0, delta=10)
        self.assertEqual(b.procs[1504].name, "Uplink")
...
class Payload(unittest.TestCase):
    def test_widget_payload(self):
        ...
        s = net.NetSampler(runner=lambda: next(outputs))
        s._interfaces = lambda now: [net.Interface("wlan0", True, 100.0, 50.0, False, "192.0.2.20"),
                                     net.Interface("proton0", True, 90.0, 40.0, True)]
        s.sample(); s._prev_ts -= 10.0
        snap = s.sample()
        p = net.to_payload(snap, {1606: "Webview", 1403: "Launcher", 1302: "Pairing"})
        self.assertEqual(p["ifaces"][1]["vpn"], True)
        self.assertEqual(p["top"][0]["name"], "Webview")
        self.assertGreater(p["top"][0]["rx"], 90000)
        doors = {d["name"]: d["ports"] for d in p["listening"]}
        self.assertEqual(doors["Launcher"], [27036])
        self.assertEqual(doors["Pairing"], [1716])
